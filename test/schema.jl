using Test, JSON, JSONSchema, Dates, StructUtils
...
@testset "JSON Schema Generation" begin
    @testset "Primitive Types" begin
        ...
        schema = JSONSchema.schema(SimpleInt)
        ...
        schema = JSONSchema.schema(SimpleFloat)
        ...
        schema = JSONSchema.schema(SimpleString)
        ...
        schema = JSONSchema.schema(SimpleBool)
    ...
    @testset "Optional Fields (Union{T, Nothing})" begin
        ...
        schema = JSONSchema.schema(OptionalFields)
    ...
    @testset "Array Types" begin
        ...
        schema = JSONSchema.schema(ArrayTypes)
    ...
    @testset "Nested Structs" begin
        ...
        schema = JSONSchema.schema(Person)
    ...
    @testset "Ignored Fields" begin
        ...
        schema = JSONSchema.schema(WithIgnored)
    ...
    @testset "Dict and Set Types" begin
        ...
        schema = JSONSchema.schema(CollectionTypes)
    ...
    @testset "Tuple Types" begin
        ...
        schema = JSONSchema.schema(WithTuple)
    ...
    @testset "Complex Union Types" begin
        ...
        schema = JSONSchema.schema(ComplexUnion)
    ...
    @testset "Schema Type" begin
        ...
        schema = JSONSchema.schema(SchemaTypeTest)

        # Test that we get a Schema{T} object
        @test schema isa JSONSchema.Schema{SchemaTypeTest}
    ...
    @testset "Schema Validation - Roundtrip" begin
        ...
        schema = JSONSchema.schema(SimpleType)
        json_str = JSON.json(schema)
    ...
    @testset "Empty Struct" begin
        ...
        schema = JSONSchema.schema(EmptyStruct)
        @test schema["type"] == "object"
        @test haskey(schema, "properties")
    ...
    @testset "Validation - String Constraints" begin
        ...
        # Valid instances
        @test JSONSchema.isvalid(schema, StringValidated("abc", "test@example.com", "hello"))
        @test JSONSchema.isvalid(schema, StringValidated("abcdefghij", "a@b.c", "abc"))

        # Invalid: name too short
        @test !JSONSchema.isvalid(schema, StringValidated("ab", "test@example.com", "hello"))

        # Invalid: name too long
        @test !JSONSchema.isvalid(schema, StringValidated("abcdefghijk", "test@example.com", "hello"))

        # Invalid: bad email
        @test !JSONSchema.isvalid(schema, StringValidated("abc", "not-an-email", "hello"))
    ...
    @testset "Validation - Numeric Constraints" begin
        ...
        # Valid instances
        @test JSONSchema.isvalid(schema, NumericValidated(25, 10.0, 5.0))
        @test JSONSchema.isvalid(schema, NumericValidated(0, 0.1, 0.5))

        # Invalid: age too high
        @test !JSONSchema.isvalid(schema, NumericValidated(200, 10.0, 5.0))

        # Invalid: age negative
        @test !JSONSchema.isvalid(schema, NumericValidated(-5, 10.0, 5.0))

        # Invalid: price must be > 0 (exclusive)
        @test !JSONSchema.isvalid(schema, NumericValidated(25, 0.0, 5.0))

        # Invalid: not a multiple of 0.5
        @test !JSONSchema.isvalid(schema, NumericValidated(25, 10.0, 5.3))
    end
    ...
    @testset "Validation - Array Constraints" begin
        ...
        # Valid instances
        @test JSONSchema.isvalid(schema, ArrayValidated(["a", "b"], [1, 2]))
        @test JSONSchema.isvalid(schema, ArrayValidated(["a"], [1, 2, 3]))

        # Invalid: tags empty (minItems=1)
        @test !JSONSchema.isvalid(schema, ArrayValidated(String[], [1, 2]))

        # Invalid: tags too many (maxItems=5)
        @test !JSONSchema.isvalid(schema, ArrayValidated(["a", "b", "c", "d", "e", "f"], [1, 2]))

        # Invalid: tags not unique
        @test !JSONSchema.isvalid(schema, ArrayValidated(["a", "a"], [1, 2]))

        # Invalid: numbers too few (minItems=2)
        @test !JSONSchema.isvalid(schema, ArrayValidated(["a"], [1]))
    end
    ...
    @testset "Validation - Enum and Const" begin
        ...
        # Valid instances
        @test JSONSchema.isvalid(schema, EnumValidated("active", "v1"))
        @test JSONSchema.isvalid(schema, EnumValidated("inactive", "v1"))
        @test JSONSchema.isvalid(schema, EnumValidated("pending", "v1"))

        # Invalid: status not in enum
        @test !JSONSchema.isvalid(schema, EnumValidated("deleted", "v1"))

        # Invalid: version doesn't match const
        @test !JSONSchema.isvalid(schema, EnumValidated("active", "v2"))
    end
    ...
    @testset "Validation - Optional Fields" begin
        ...
        @test JSONSchema.isvalid(schema, OptionalValidated("test", nothing))

        # Valid: both fields present and valid
        @test JSONSchema.isvalid(schema, OptionalValidated("test", "hello"))

        # Invalid: required field empty
        @test !JSONSchema.isvalid(schema, OptionalValidated("", nothing))

        # Invalid: optional field present but too short
        @test !JSONSchema.isvalid(schema, OptionalValidated("test", "hi"))
    end
    ...
    @testset "Validation - Nested Structs" begin
        ...
        schema = JSONSchema.schema(OuterValidated)

        # Valid instance
        @test JSONSchema.isvalid(schema, OuterValidated("test", InnerValidated(5)))

        # Invalid: outer field fails
        @test !JSONSchema.isvalid(schema, OuterValidated("", InnerValidated(5)))

        # Invalid: inner field fails
        @test !JSONSchema.isvalid(schema, OuterValidated("test", InnerValidated(0)))
        @test !JSONSchema.isvalid(schema, OuterValidated("test", InnerValidated(11)))
    end
    ...
    @testset "Validation - Verbose Mode" begin
        ...
        @test !JSONSchema.isvalid(schema, invalid)
    ...
    @testset "Validation - Complex Real-World Example" begin
        ...
        @test JSONSchema.isvalid(schema, valid_product)
...
@testset "Negation - not Combinator" begin
    ...
    @test JSONSchema.isvalid(schema, ExcludedStatus("active"))
    @test JSONSchema.isvalid(schema, ExcludedStatus("pending"))

    # Invalid: status is in the excluded list
    @test !JSONSchema.isvalid(schema, ExcludedStatus("deleted"))
    @test !JSONSchema.isvalid(schema, ExcludedStatus("archived"))
    ...
    # Valid: not a string
    @test JSONSchema.isvalid(schema2, NotStringValue(42))
    @test JSONSchema.isvalid(schema2, NotStringValue(true))
    @test JSONSchema.isvalid(schema2, NotStringValue(nothing))
    ...
    @test JSONSchema.isvalid(schema3, ExcludedRange(5))
    @test JSONSchema.isvalid(schema3, ExcludedRange(25))

    # Invalid: inside the excluded range
    @test !JSONSchema.isvalid(schema3, ExcludedRange(10))
    @test !JSONSchema.isvalid(schema3, ExcludedRange(15))
    @test !JSONSchema.isvalid(schema3, ExcludedRange(20))
end
...
@testset "Array Contains" begin
    ...
    @test JSONSchema.isvalid(schema, TaskWithPriority(["urgent", "bug"]))
    @test JSONSchema.isvalid(schema, TaskWithPriority(["feature", "important"]))
    @test JSONSchema.isvalid(schema, TaskWithPriority(["critical"]))
    @test JSONSchema.isvalid(schema, TaskWithPriority(["urgent", "important", "critical"]))

    # Invalid: no priority tags
    @test !JSONSchema.isvalid(schema, TaskWithPriority(["bug", "feature"]))
    @test !JSONSchema.isvalid(schema, TaskWithPriority(["normal"]))
    @test !JSONSchema.isvalid(schema, TaskWithPriority(String[]))
    ...
    @test JSONSchema.isvalid(schema2, EmailList(["admin@example.com", "user@example.com"]))
    @test JSONSchema.isvalid(schema2, EmailList(["admin@test.com"]))

    # Invalid: no admin emails
    @test !JSONSchema.isvalid(schema2, EmailList(["user@example.com"]))
    ...
    @test JSONSchema.isvalid(schema3, NumberList([50, 100, 150]))
    @test JSONSchema.isvalid(schema3, NumberList([200]))

    # Invalid: all numbers < 100
    @test !JSONSchema.isvalid(schema3, NumberList([50, 75, 99]))
end
...
@testset "Tuple Validation - Automatic" begin
    ...
    schema = JSONSchema.schema(Point2D)
    ...
    # Valid tuples
    @test JSONSchema.isvalid(schema, Point2D((1.0, 2.0)))
    @test JSONSchema.isvalid(schema, Point2D((0.0, 0.0)))
    @test JSONSchema.isvalid(schema, Point2D((-5.5, 10.7)))
    ...
    @test JSONSchema.isvalid(schema2, LatLon((45.0, -122.0)))
    @test JSONSchema.isvalid(schema2, LatLon((0.0, 0.0)))
    @test JSONSchema.isvalid(schema2, LatLon((90.0, 180.0)))
    @test JSONSchema.isvalid(schema2, LatLon((-90.0, -180.0)))

    # Invalid: latitude out of range
    @test !JSONSchema.isvalid(schema2, LatLon((95.0, 0.0)))
    @test !JSONSchema.isvalid(schema2, LatLon((-95.0, 0.0)))

    # Invalid: longitude out of range
    @test !JSONSchema.isvalid(schema2, LatLon((0.0, 190.0)))
    @test !JSONSchema.isvalid(schema2, LatLon((0.0, -190.0)))
    ...
    schema3 = JSONSchema.schema(MixedTuple)
    ...
    # Valid mixed tuple
    @test JSONSchema.isvalid(schema3, MixedTuple(("hello", 42, true)))
    ...
    # Valid RGB values
    @test JSONSchema.isvalid(schema4, RGB((255, 0, 0)))     # Red
    @test JSONSchema.isvalid(schema4, RGB((0, 255, 0)))     # Green
    @test JSONSchema.isvalid(schema4, RGB((0, 0, 255)))     # Blue
    @test JSONSchema.isvalid(schema4, RGB((128, 128, 128))) # Gray

    # Invalid: values out of range
    @test !JSONSchema.isvalid(schema4, RGB((256, 0, 0)))
    @test !JSONSchema.isvalid(schema4, RGB((0, -1, 0)))
    @test !JSONSchema.isvalid(schema4, RGB((0, 0, 300)))
end
...
@testset "Combined Advanced Features" begin
    ...
    @test JSONSchema.isvalid(schema, AdvancedValidation(["urgent", "bug"], (1.0, 2.0)))

    # Invalid: no priority tag
    @test !JSONSchema.isvalid(schema, AdvancedValidation(["bug"], (1.0, 2.0)))
    ...
    @test JSONSchema.isvalid(schema2, ComplexNot(5))
    @test JSONSchema.isvalid(schema2, ComplexNot(25))
    @test JSONSchema.isvalid(schema2, ComplexNot(100))

    # Invalid: in the excluded range
    @test !JSONSchema.isvalid(schema2, ComplexNot(10))
    @test !JSONSchema.isvalid(schema2, ComplexNot(15))
    @test !JSONSchema.isvalid(schema2, ComplexNot(20))
...
@testset "Schema References (\$ref)" begin
    @testset "Simple Refs - Basic Usage" begin
        ...
        @defaults struct RefAddress
            street::String = ""
            city::String = ""
            zip::String = ""
        end

        @defaults struct RefPerson
            name::String = ""
            address::RefAddress = RefAddress()
        end

        # Test without refs (default behavior - inlined)
        schema_inline = JSONSchema.schema(RefPerson)
    ...
    @testset "Circular References" begin
        ...
        schema_inline = JSONSchema.schema(RefUser)
    ...
    @testset "Type Deduplication" begin
        @defaults struct RefTag
            name::String = ""
        end

        @defaults struct RefPost
            title::String = ""
    ...
    @testset "Nested Refs - Three Levels Deep" begin
        @defaults struct RefLevel3
            value::String = ""
        end

        @defaults struct RefLevel2
            data::RefLevel3 = RefLevel3()
        end

        @defaults struct RefLevel1
    ...
    @testset "Complex Circular - BlogPost Example" begin
        ...
        @defaults struct RefBlogComment
            id::Int = 0
            text::String = ""
            author_id::Int = 0
        end

        @defaults struct RefBlogAuthor
            id::Int = 0
            name::String = ""
            posts::Vector{Int} = Int[]  # Just IDs to avoid deeper circular
        end

        @defaults struct RefBlogPost
            title::String = ""
        ...
        @test JSONSchema.isvalid(schema, post)
    end
    ...
    @testset "Type Name Generation" begin
        ...
        @defaults struct RefSimpleType
            x::Int = 0
        end

        @defaults struct RefContainer
...
@testset "Conditional Schemas (if/then/else)" begin
    ...
    @testset "if/then/else" begin
        ...
        @test JSONSchema.isvalid(schema, Dict("type" => "number", "value" => 42))
        @test !JSONSchema.isvalid(schema, Dict("type" => "number", "value" => "hello"))

        # If type is not "number", value must be a string
        @test JSONSchema.isvalid(schema, Dict("type" => "text", "value" => "hello"))
        @test !JSONSchema.isvalid(schema, Dict("type" => "text", "value" => 42))
    end
end
...
@testset "Advanced Object Validation" begin
    @testset "propertyNames - struct" begin
        ...
        schema = JSONSchema.Schema{PropNamesTest}(PropNamesTest, schema_obj, nothing)

        # Valid: all property names match pattern
        valid_instance = PropNamesTest("test", 42)
        @test JSONSchema.isvalid(schema, valid_instance)
    end
    ...
    @testset "propertyNames - Dict" begin
        ...
        @test JSONSchema.isvalid(schema, Dict("FOO" => 1, "BAR" => 2))
    ...
    @testset "patternProperties - Dict" begin
        ...
        @test JSONSchema.isvalid(schema, Dict("str_name" => "hello", "num_count" => 42))

        # Invalid: str_ key with number value
        @test !JSONSchema.isvalid(schema, Dict("str_name" => 123))

        # Invalid: num_ key with string value
        @test !JSONSchema.isvalid(schema, Dict("num_count" => "hello"))

        # Valid: non-matching keys are not validated
        @test JSONSchema.isvalid(schema, Dict("other" => [1, 2, 3]))
    end

    @testset "dependencies - array form (struct)" begin
        @defaults struct DepsTest
        ...
        schema = JSONSchema.Schema{DepsTest}(DepsTest, schema_obj, nothing)

        # Valid: credit_card present with required dependencies
        @test JSONSchema.isvalid(schema, DepsTest("1234", "123 Main St", "999"))

        # Valid: credit_card absent
        @test JSONSchema.isvalid(schema, DepsTest(nothing, nothing, nothing))

        # Invalid: credit_card present but missing billing_address
        @test !JSONSchema.isvalid(schema, DepsTest("1234", nothing, "999"))
    end
    ...
    @testset "dependencies - array form (Dict)" begin
        ...
        @test JSONSchema.isvalid(schema, Dict("credit_card" => "1234", "billing_address" => "123 Main"))

        # Valid: no credit_card
        @test JSONSchema.isvalid(schema, Dict("name" => "Alice"))

        # Invalid: credit_card without billing_address
        @test !JSONSchema.isvalid(schema, Dict("credit_card" => "1234"))
    end
    ...
    @testset "dependencies - schema form" begin
        ...
        @test JSONSchema.isvalid(schema, Dict("name" => "Alice", "age" => 30, "birth_year" => 1994))

        # Valid: no age
        @test JSONSchema.isvalid(schema, Dict("name" => "Bob"))

        # Invalid: age present without birth_year
        @test !JSONSchema.isvalid(schema, Dict("name" => "Carol", "age" => 25))
    end

    @testset "additionalProperties - struct (false)" begin
        @defaults struct StrictStruct
            name::String = ""
            age::Int = 0
        end
        ...
        schema = JSONSchema.Schema{StrictStruct}(StrictStruct, schema_obj, nothing)
        ...
        @test !JSONSchema.isvalid(schema, StrictStruct("Alice", 30))
    end

    @testset "additionalProperties - struct (schema)" begin
        @defaults struct FlexStruct
            name::String = ""
            extra1::Int = 0
        end
        ...
        schema = JSONSchema.Schema{FlexStruct}(FlexStruct, schema_obj, nothing)

        # Valid: extra1 is integer (matches additionalProperties)
        @test JSONSchema.isvalid(schema, FlexStruct("Alice", 42))
    end
end
...
@testset "Advanced Array Validation (additionalItems)" begin
    @testset "additionalItems - false" begin
        ...
        @test JSONSchema.isvalid(schema, ["hello", 42])

        # Invalid: more than 2 items
        @test !JSONSchema.isvalid(schema, ["hello", 42, "extra"])
    end
    ...
    @testset "additionalItems - schema" begin
        ...
        @test JSONSchema.isvalid(schema, ["hello", 42, true, false])

        # Invalid: additional item is not boolean
        @test !JSONSchema.isvalid(schema, ["hello", 42, "not a boolean"])
    ...
    @testset "additionalItems with no items constraint" begin
        ...
        @test JSONSchema.isvalid(schema, ["hello", "world", "foo"])
    end
end
...
@testset "Validation API and Formats" begin
    ...
    @testset "Improved Format Validation" begin
        ...
        # Email
        @test JSONSchema.isvalid(schema, FormatTestV2("test@example.com", "http://a.com", "2023-01-01T12:00:00Z"))
        @test !JSONSchema.isvalid(schema, FormatTestV2("test @example.com", "http://a.com", "2023-01-01T12:00:00Z"))
        @test !JSONSchema.isvalid(schema, FormatTestV2("test", "http://a.com", "2023-01-01T12:00:00Z"))

        # URI
        @test JSONSchema.isvalid(schema, FormatTestV2("a@b.c", "ftp://example.com", "2023-01-01T12:00:00Z"))
        @test JSONSchema.isvalid(schema, FormatTestV2("a@b.c", "mailto:user@host", "2023-01-01T12:00:00Z"))
        @test !JSONSchema.isvalid(schema, FormatTestV2("a@b.c", "example.com", "2023-01-01T12:00:00Z"))
        @test !JSONSchema.isvalid(schema, FormatTestV2("a@b.c", "http://exa mple.com", "2023-01-01T12:00:00Z"))

        # Date-time
        @test JSONSchema.isvalid(schema, FormatTestV2("a@b.c", "http://a.com", "2023-01-01T12:00:00Z"))
        @test JSONSchema.isvalid(schema, FormatTestV2("a@b.c", "http://a.com", "2023-01-01T12:00:00+00:00"))
        @test JSONSchema.isvalid(schema, FormatTestV2("a@b.c", "http://a.com", "2023-01-01T12:00:00.123Z"))
        @test !JSONSchema.isvalid(schema, FormatTestV2("a@b.c", "http://a.com", "2023-01-01T12:00:00")) # No timezone
        @test !JSONSchema.isvalid(schema, FormatTestV2("a@b.c", "http://a.com", "2023/01/01"))
    end
end
